    ...
    if (winner != true) {
      if (this.classList.contains("clicked")) {
        ...
        winner
          ...
          ? currentPlayer++
          : currentPlayer--;
        currentPlayerIndicator.innerText = tieGame
          ? "CATS GAME"
          : "Player " + (currentPlayer + 1) + (winner ? " WINS!" : "");
        scoreboard.innerText =
          "SCORE \n\nPLAYER 1: " +
          playerOneWins +
          "\n\nPLAYER 2: " +
...
function catsGame() {
  ...
  if (filled && !winner) container.classList.add("cats-game");
}
...
function showWinner() {
  ...
  if (winner) container.classList.add("winner");
...
function testRows() {
  for (let i = 0; i <= 8; i) {
    ...
    let row =
      gameBlocks[i].innerText +
      gameBlocks[i + 1].innerText +
      gameBlocks[i + 2].innerText;
    if (row == "XXX" || row == "OOO") {
      rows = true;
...
function testColumns() {
  for (let i = 0; i < 3; i++) {
    ...
    column =
      gameBlocks[i].innerText +
      gameBlocks[i + 3].innerText +
      gameBlocks[i + 6].innerText;
